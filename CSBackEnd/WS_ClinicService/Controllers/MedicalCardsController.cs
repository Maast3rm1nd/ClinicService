...
namespace WS_ClinicService.Controllers
{
    ...
    public class MedicalCardsController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(MedicalCardSnapshotDto), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<MedicalCardSnapshotDto>> RestoreMedicalCard(Guid entityId, CancellationToken cancellationToken)
            => Ok(await _mediator.Send(new RestoreMedicalCardCommand(entityId), cancellationToken));
    }
}